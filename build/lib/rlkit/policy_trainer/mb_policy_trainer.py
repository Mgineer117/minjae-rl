...
class MBPolicyTrainer:
    ...
    def train(self) -> Dict[str, float]:
        ...
        for e in trange(self._epoch, desc=f"Epoch"):
            ...
            for it in trange(self._step_per_epoch, desc=f"Training", leave=False):
                if num_timesteps % self._rollout_freq == 0:
                    ...
                    for _key, _value in rollout_info.items():
                        self.logger.store({"rollout_info/"+_key: _value})
            ...
            eval_data = {"eval/normalized_episode_reward": norm_ep_rew_mean,
        ...
        self.policy.dynamics.save(self.logger.log_dir)
    ...
    def _evaluate(self) -> Dict[str, List[float]]:
        ...
        while num_episodes < self._eval_episodes:
            action = self.policy.select_action(self.mask_obs(obs).reshape(1, -1), deterministic=True)
