...
from rlkit.dynamics import BaseDynamics
...
class EnsembleDynamics(BaseDynamics):
    ...
    def train(
        ...
        epoch = 0
        cnt = 0
        logger.print("Training dynamics:")
        while True:
            epoch += 1
            ...
            to_log = {"loss/dynamics_train_loss": train_loss,
        ...
        self.save(logger.log_dir)
        self.model.eval()
        logger.print("elites:{} , holdout loss: {}".format(indexes, (np.sort(holdout_losses)[:self.model.num_elites]).mean()))
