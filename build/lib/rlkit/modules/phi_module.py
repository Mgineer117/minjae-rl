...
class PhiNetwork(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        self.latent_dim = kwargs['latent_dims']
    ...
    def update_context(self, inputs):
        ...
        o = torch.from_numpy(o[None, None, ...])
        a = torch.from_numpy(a[None, None, ...])
        r = torch.from_numpy(np.array([r])[None, None, ...])
        no = torch.from_numpy(no[None, None, ...])
    ...
    def log_diagnostics(self, eval_statistics):
        ...
        '''
        z_mean = np.mean(np.abs(torch.get_numpy(self.z_means[0])))
        z_sig = np.mean(torch.get_numpy(self.z_vars[0]))
