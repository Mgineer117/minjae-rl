...
def get_args():
    ...
    parser.add_argument("--algo-name", type=str, default="optimaml")
...
def collect_buffers(args):
    buffers = []
    assert args.num_task > 10
    for i in range(args.num_task + 1):
        if i == args.num_task:
            # test data
            file_name = args.task + '.h5py'
            hfile_dir = os.join(('data', args.task, 'test', file_name))
        else:
            # train data
            file_name = args.task + '_' + str(i) + '.h5py'
            hfile_dir = os.join(('data', args.task, 'train', file_name))
...
def train(param, args=get_args()):
    ...
    for seed in args.seeds:
        ...
        env = gym.make(args.task + '-v4')
        reward_fn = load_reward_fn('Gym-' + args.task)
