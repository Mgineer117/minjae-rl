...
def collect_gym_buffers(args):
    ...
    for i in range(args.num_task + 1):
        if i == args.num_task:
            ...
            hfile_dir = os.join(('data', args.env_type, args.agent_type, 'test', file_name))
        else:
            # train data
            file_name = args.agent_type + '_train_' + str(i) + '.h5py'
            hfile_dir = os.join(('data', args.env_type, args.agent_type, 'train', file_name))
...
def collect_metagym_buffers(args):
    ...
    for i in range(args.num_task + 1):
        ...
        hfile_dir = os.join(('data', args.env_type, file_name))
