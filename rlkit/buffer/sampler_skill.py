...
class OnlineSkillSampler:
    ...
    def collect_blind_trajectory(self, pid, queue, env, policy, thread_batch_size, episode_len,
        ...
        while current_step < thread_batch_size:
            ...
            while t < episode_len:
                # sample action
                with torch.no_grad():
                    a, logprob = policy.blind_select_action(e_s, deterministic=deterministic)
                # env stepping
                try:
    ...
    def to_device(self, policy, device=torch.device('cpu')):
        ...
        policy.blind_actor.device = device
        policy.encoder.device = device
        return policy

    def collect_samples(self, policy, seed, deterministic=False, is_blind=False):
        '''
